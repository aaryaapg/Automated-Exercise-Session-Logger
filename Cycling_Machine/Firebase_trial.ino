...
#include <Wire.h>
#include<math.h>
//For Occupancy (I'm using FSR for time being)
#include <SPC.h>
...
/* ______________________________________Declarations and Variables: Occupancy______________________________________ */
...
void startSession(){
  ...
  Firebase.setString(firebaseData, path + "/cycle/Login", TIMESTAMP);
  Firebase.setString(firebaseData, path + "/cycle/Date", DATE + ", " + WEEKDAY);
...
void readSensors() {
  int Occ_counter=0; //Occupancy Counter
  while(1) {
    fsrRaw = analogRead(FSR_Pin);
    delay(100);
    if(spc.SPC_getSquare(200, fsrRaw)==LOW) { //Nobody on the cycle
      delay(2000); 
      Serial.println("40 seconds timer");
      Occ_counter = Occ_counter+1;
      if(Occ_counter > 20) { //Logout Procedure
        statuss=0;
        timeClient.update();
        logout_time = timeClient.getEpochTime() + (19800); //5 hours 30 minutes = 19800s (India is GMT+5:30)
        getTimestamp(logout_time);
        int Reps = realcount;
        Firebase.setInt(firebaseData, path + "/cycle/Count", Reps);
        Firebase.setString(firebaseData, path + "/cycle/Logout", TIMESTAMP);
        ...
        Firebase.setDouble(firebaseData, path + "/cycle/Duration", duration);
        Firebase.setBool(firebaseData, path + "/cycle/Ongoing_Session", false);
        delay(500);  
        path="/";
    ...
    else if(spc.SPC_getSquare(200, fsrRaw)==HIGH) { 
